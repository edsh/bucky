...
- [X] T002 Die Sollwerte für die Prüfdaten in dieser Datei festhalten (Tabelle
...
- [X] T004 Den Ordner `packages/deelk-poh-core/src/wind/` anlegen und in
...
- [X] T005 [P] `packages/deelk-poh-core/tests/runwayComponent.test.ts` anlegen
...
- [X] T006 [P] `packages/deelk-poh-core/tests/temperature.test.ts` um den
...
- [X] T007 `toIsaDeviation(pressureAltitudeFt, outsideAirTemperatureC)` in
...
- [X] T008 `toRunwayWindComponent(windFromDegTrue, windSpeedKt, runwayBearingDegTrue)`
...
- [X] T009 Beide Funktionen und ihre Typen in
...
- [X] T011 In `apps/web/src/lib/weather/edsh.ts` die Bahnrichtungen ergänzen:
...
- [X] T012 In `apps/web/src/lib/weather/openMeteo.ts` die Anfrage erweitern:
...
- [X] T013 `WetterAbruf` in derselben Datei um `temperatureC?: number` und
      `wind?: { fromDegTrue: number; speedKt: number }` erweitern.

- [X] T014 `deuteAntwort` erweitern: Luftdruck, Zeit und Bezugshöhe bleiben
...
- [X] T015 `apps/web/src/lib/components/QnhAbrufDialog.svelte` nach
...
- [X] T016 Die Props umstellen: statt eines `bereich` und eines `uebernehmen`
...
- [X] T017 Den Zustand `vorschau` umbauen: Er trägt jetzt den `WetterAbruf` und
...
- [X] T018 Die drei Vorschläge bilden:
...
- [X] T019 Die Übernehmbarkeit je Vorschlag gegen den zugehörigen Bereich
...
- [X] T020 Die Vorauswahl der Bahn: diejenige mit der größeren Längskomponente.
...
- [X] T021 Die Bahnwahl im Markup: zwei Optionen, beschriftet mit den
      Kennungen „10" und „28", gerechnet mit den Gradzahlen. Sichtbar nur im
      Zustand `vorschau`.

- [X] T022 Die drei Zeilen im Markup: je ein `<input type="checkbox">` mit
...
- [X] T023 Gesperrte Zeilen: Kästchen `disabled` und nicht angehakt, dazu die
      begründende Meldung. Die übrigen Zeilen bleiben bedienbar (FR-007).

- [X] T024 „Übernehmen" sperren, solange kein Ergebnis vorliegt **oder** kein
      Kästchen angehakt ist (FR-005). `bestaetigen()` gibt nur die angehakten
      Größen weiter (FR-004).

- [X] T025 Die Aufklärung, die Namensnennung, die Ladeanzeige, die Zeitgrenze,
...
- [X] T026 In `apps/web/src/routes/+page.svelte` den Import und die Verwendung
...
- [X] T027 Aus `qnhHerkunft` drei Zustände machen: `qnhHerkunft`,
      `isaHerkunft`, `pistenwindHerkunft` (→ [E6](./plan.md)).

- [X] T028 Die Übernahme umstellen: `wetterUebernehmen(werte, herkunft)` setzt
...
- [X] T029 Drei `bedient`-Wächter statt einem: `qnhVonHand`, `isaVonHand`,
...
- [X] T030 Den `folge`-Steckplatz am ISA-Regler ergänzen und den
...
- [X] T031 Prüfen, dass die Streckenwindkomponente nirgends berührt wird
      (FR-020, W-10).
...
- [X] T032 In `tests/ui/klickpfad.mjs` die Prüfdaten erweitern: `GUTE_ANTWORT`
      um `temperature_2m: 29.2`, `wind_speed_10m: 12`,
      `wind_direction_10m: 250`. Die erwarteten Werte aus der Tabelle
      „Sollwerte" unten.

- [X] T033 Die Prüfungen 40 bis 55 auf die neuen Testkennungen und den neuen
...
- [X] T034 Prüfung 44 erweitern: drei Zeilen, drei angehakte Kästchen, die drei
      erwarteten Vorschauwerte.

- [X] T035 Prüfung 45 erweitern: „Übernehmen" setzt alle drei Regler; die
      Streckenwindkomponente bleibt auf ihrem Wert.

- [X] T036 Prüfung 46 erweitern: drei Herkunftsvermerke; 48 erweitern: das
      Verstellen **eines** Reglers löscht nur dessen Vermerk.

- [X] T037 Prüfung 54 umstellen: Statt eines QNH außerhalb des Bereichs eine
...
- [X] T038 [P] Neue Prüfung: ein abgewähltes Kästchen lässt seinen Regler und
      dessen bisherigen Vermerk unverändert (US2, SC-002).

- [X] T039 [P] Neue Prüfung: alle drei Kästchen abgewählt → „Übernehmen"
      gesperrt (FR-005).

- [X] T040 [P] Neue Prüfung: ein Bahnwechsel ändert allein den
      Pistenwind-Vorschauwert und löst **keine** zweite Anfrage aus — über einen
      Zähler auf dem abgefangenen Aufruf (US3, FR-011).

- [X] T041 [P] Neue Prüfung: die Bahn mit Gegenwind ist vorausgewählt (FR-010).

- [X] T042 [P] Neue Prüfung: eine Antwort ohne Windfelder lässt QNH und ISA
      übernehmbar und sperrt allein den Pistenwind (FR-007).

- [X] T043 [P] Neue Prüfung: die Erläuterungen nennen Windrichtung,
      Windgeschwindigkeit, Bahn und die absolute Platztemperatur (US4, FR-012,
      FR-013).

- [X] T044 Den Klickpfad vollständig laufen lassen: `npm run build`, Server auf
...
- [X] T045 `README.md` um einen Abschnitt ergänzen, der beschreibt, was der
...
- [X] T046 Die drei Prüfläufe aus [quickstart.md](./quickstart.md) Abschnitt 1,
      2 und 4 abschließend ausführen und das Ergebnis unten unter „Endstand der
      Prüfungen" eintragen.

- [X] T047 Alle Aufgaben in dieser Datei auf `[X]` setzen und Abweichungen von
...
**A1 — `roundTo` lieferte `NaN` für sehr kleine Beträge (Fund, nicht geplant).**
Der Test „Wind genau quer zur Bahn" schlug mit `NaN` fehl. Ursache war eine
bestehende Schwäche in `format.ts`: `Number(\`${value}e0\`)` ergibt für eine
Zahl, die JavaScript bereits exponentiell schreibt, die Zeichenkette
`"6.1e-17e0"` und daraus `NaN`. Der Fall war vorher unerreichbar — der Kosinus
eines rechten Winkels erzeugt genau solche Werte, und den gab es bis zu diesem
Feature nicht. `roundTo` behandelt den Exponenten jetzt getrennt. Derselbe
Eingriff befreit das Ergebnis von negativer Null, die sonst als „−0 kt"
erschiene.

**A2 — Die Adaptertests zu `openMeteo` mussten mitgezogen werden.** In der
Aufgabenliste nicht vorgesehen: `apps/web/tests/weather/openMeteo.test.ts`
prüfte wörtlich, dass **nur** der Stationsdruck angefordert wird. Der Test
wurde umgeschrieben und um die Fälle zu Temperatur, Wind, halber Windangabe
und falscher Einheit ergänzt (jetzt 27 statt 15 Prüfungen).

**A3 — Die Einheit des Windes wird gegengeprüft.** Über T014 hinaus: Der Wind
wird nur übernommen, wenn `current_units.wind_speed_10m` tatsächlich `"kn"`
lautet. Das ist keine Plausibilitätsschranke im Sinne von C-05, sondern eine
Einheitenprüfung — fiele der Dienst stillschweigend auf km/h zurück, ergäben
22 km/h einen Wind von 22 kt, mehr als das Doppelte, und die Zahl sähe dabei
unverdächtig aus.

**A4 — Die Platztemperatur steht ungerundet in der Erläuterung.** T022 nannte
`formatCelsius`; damit erschienen die abgerufenen 29,2 °C als „29 °C", und
zusammen mit der Abweichung von 16 °C ließ sich die Rechnung nicht
nachvollziehen. Jetzt `formatNumber(temperatur, 1)` — dieselbe Linie wie beim
ungerundeten QNH eine Zeile darüber.

**A5 — Der Pistenwind-Vermerk wird als fertiger Text durchgereicht.** T030
sprach von „Vermerk und Wächter als Props". Umgesetzt als `windHerkunft:
string` statt als Herkunftsobjekt: Sonst müsste `TakeoffDistance.svelte` die
Zeitangabe selbst formatieren und trüge damit eine zweite Fassung derselben
Darstellung.

**A6 — Neue Klickpfad-Prüfungen tragen die Nummern 58 bis 63.** Die
Aufgabenliste (T038–T043) nannte keine Nummern; 56 und 57 waren von Feature
026 belegt. Die Reihenfolge im Ablauf weicht von der Nummernfolge ab, weil die
Fehlerfälle (51–53) ans Ende der Abrufstrecke gehören — sie hinterlassen eine
abgebrochene Route.

**A7 — `Uebernahmewerte` steht im `<script module>`-Block.** In Svelte 5 lässt
sich aus dem Instanzblock kein Typ ausführen. Der Modulblock ist ohnehin die
richtige Stelle: Ein Typ hat mit einer einzelnen Einbindung des Dialogs nichts
zu tun.
...
| Prüflauf | Ergebnis |
|---|---|
| `npx vitest run` | **492 Tests, 19 Dateien, alle grün** (vorher 436) |
| `npm run lint` | **ohne Befund** |
| `svelte-check` | **0 Fehler, 0 Warnungen** |
| `npm run build` | **grün**, statisches Bündel geschrieben |
| `node tests/ui/klickpfad.mjs` | **63 Prüfungen, 0 durchgefallen** (vorher 57) |

Die Sollwerte aus der Tabelle oben sind im Klickpfad bestätigt: QNH 1023 hPa,
ISA-Abweichung 16 °C, Pistenwind +10 kt auf Bahn 28 und −10 kt auf Bahn 10 —
letzteres übernehmbar, weil es genau auf der Reglergrenze liegt. Erst 20 kt aus
250° ergeben mit −17 kt den Sperrfall (Prüfung 54).
